    ...
    def buscar_previsao_clima(self, lat: float, lon: float) -> list:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1: print(f" -> Falha na previsão diária: {e}")
                time.sleep(2)
        return []
    ...
    def buscar_previsao_horaria(self, lat: float, lon: float) -> list:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1: print(f" -> Falha na previsão horária: {e}")
                time.sleep(2)
        return []
...
<body>
    <div class="container">
        ...
        <div id="tabRadiacao" class="tab-content">
            <div class="kpi-grid">
                <div class="kpi-card"><h4>Radiação Acumulada</h4><div class="value" id="kpi-rad-total">0,0</div><span style="color:#8892b0;">MJ/m²</span></div>
                <div class="kpi-card"><h4>Média Diária</h4><div class="value" id="kpi-rad-media">0,0</div><span style="color:#8892b0;">MJ/m²/dia</span></div>
        ...
        <div id="tabPrevisao" class="tab-content">
            ...
            <div id="forecast-table-container">
                ...
                <table class="forecast-table">
                    <thead><tr><th>Dia</th><th>Mínima</th><th>Máxima</th><th>Condição</th><th>Prob. de Precip.</th><th>Quantidade</th><th>Vento (km/h)</th></tr></thead>
    ...
    <script>
        ...
        function renderForecastTable(station) {
            ...
            if (!forecastData || forecastData.length === 0) { tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center;">Não foi possível carregar a previsão do tempo.</td></tr>'; return; }
        ...
        function renderHourlyForecast(station) {
            ...
            if (!hourlyData || hourlyData.length === 0) { container.innerHTML = '<p style="text-align:center;">Não foi possível carregar a previsão horária.</p>'; return; }
            ...
            const createHourlyBar = (data, title) => {
                ...
                for (let h = 0; h < 24; h++) {
                    ...
                    hourDiv.innerHTML = `<div class="spray-hour-content"><div class="spray-hour-time">${h}h</div><div class="spray-hour-value">🌡️ ${tempText}</div><div class="spray-hour-value">🌬️ ${windText}</div><div class="spray-hour-value">ΔT: ${deltaTText}</div></div><span class="tooltip-text"><b>Hora: ${String(h).padStart(2,'0')}:00</b><br>Temp: ${tempText}<br>Umidade: ${rh !== null ? fNum(rh,0) + '%' : 'N/D'}<br>Vento: ${windText}<br>ΔT: ${deltaTText} °C<br>Chuva: ${pop !== null ? fNum(pop,0) + '%' : 'N/D'}</span>`;
            ...
            createHourlyBar(todayData, 'Hoje'); createHourlyBar(tomorrowData, 'Amanhã');
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function generateAndRenderHistoricalAlerts(hourlyData) {
                ...
                for (const key in dailyDataByStation) {
                    ...
                    if (dayData.precip > ALERT_THRESHOLDS.RAIN_LIMIT) { alertsByMonth[month].push({type: 'rain', date: dayData.date, icon: '🌧️', title: 'Chuva Volumosa', description: `Acumulado de <strong>${fNum(dayData.precip)} mm</strong> no dia.`, station: dayData.station }); }
                    if (maxGust > ALERT_THRESHOLDS.GUST_LIMIT) { alertsByMonth[month].push({type: 'gust', date: dayData.date, icon: '💨', title: 'Rajada de Vento Forte', description: `Rajada máxima de <strong>${fNum(maxGust,0)} km/h</strong> registrada.`, station: dayData.station}); }
                    if (maxTemp > ALERT_THRESHOLDS.TEMP_HIGH) { alertsByMonth[month].push({type: 'temp_high', date: dayData.date, icon: '🌡️', title: 'Temperatura Alta', description: `Máxima de <strong>${fNum(maxTemp)} °C</strong> registrada.`, station: dayData.station}); }
                    if (minTemp < ALERT_THRESHOLDS.TEMP_LOW) { alertsByMonth[month].push({type: 'temp_low', date: dayData.date, icon: '🧊', title: 'Temperatura Baixa', description: `Mínima de <strong>${fNum(minTemp)} °C</strong> registrada.`, station: dayData.station}); }
                    if (minHum < ALERT_THRESHOLDS.HUM_LOW) { alertsByMonth[month].push({type: 'hum_low', date: dayData.date, icon: '💧', title: 'Umidade Baixa', description: `Umidade relativa mínima de <strong>${fNum(minHum,0)}%</strong>.`, station: dayData.station}); }
                }
                ...
                if (sortedMonths.length === 0) { container.innerHTML = '<div class="alert-section"><h2>Alertas Históricos</h2><p style="text-align:center; padding: 20px;">Nenhum alerta climatológico significativo encontrado para o período selecionado.</p></div>'; return; }
            ...
            function generateAndRenderFutureAlerts() {
                ...
                for(const key in deltaTAlerts) { const [stationName, dateStr] = key.split('|'); const hours = deltaTAlerts[key].sort((a,b) => a-b).map(h => h + 'h').join(', '); allAlerts.push({ station: stationName, type: 'delta_t', date: dateStr, icon: '☀️', title: 'Delta T Elevado', description: `Condições de Delta T acima de <strong>${ALERT_THRESHOLDS.DELTA_T_HIGH}°C</strong> previstas para as horas: <strong>${hours}</strong>.` }); }
                if (allAlerts.length === 0) return;
                let finalHtml = '<div class="alert-section"><h2>Alertas Previstos</h2><div class="alert-grid">';
                allAlerts.sort((a, b) => a.date.localeCompare(b.date) || a.station.localeCompare(b.station)).forEach(alert => { finalHtml += `<div class="alert-card alert-card-${alert.type}"><div class="alert-icon">${alert.icon}</div><div class="alert-body"><h4>${alert.title}</h4><p><strong>Data:</strong> ${alert.date}</p><p>${alert.description}</p><p><strong>Estação:</strong> ${alert.station}</p></div></div>`; });
